...
enum StateType{TYPE_UNKNOWN = -1, igenic, 
	       ...
	       utr5termHigh, utr5termMedium, utr5termLow, // 5'UTR states
	       ...
	       utr3termHigh, utr3termMedium, utr3termLow, // 3'UTR states
	       ...
	       rutr5termHigh, rutr5termMedium, rutr5termLow, // 5'UTR states
	       ...
	       rutr3termHigh, rutr3termMedium, rutr3termLow, // 3'UTR states
	       
	       intron_type, rintron_type, exon_type,
	       
	       // new exon states for forward strand
	       singleGHigh, singleGMedium, singleGLow, 
	       ...
	       // new exon states for reverse strand
	       rsingleGHigh, rsingleGMedium, rsingleGLow,
...
inline Boolean isInitialExon(StateType type){
    return (type==initial0High || type==initial1High || type==initial2High || type==initial0Medium || type==initial1Medium || type==initial2Medium || type==initial0Low || type==initial1Low || type==initial2Low);
}

inline Boolean isInternalExon(StateType type){
    return (type==internal0High || type==internal1High || type==internal2High || type==internal0Medium || type==internal1Medium || type==internal2Medium || type==internal0Low || type==internal1Low || type==internal2Low);
}

inline Boolean isRTerminalExon(StateType type){
    return (type==rterminal0High || type==rterminal1High || type==rterminal2High || type==rterminal0Medium || type==rterminal1Medium || type==rterminal2Medium || type==rterminal0Low || type==rterminal1Low || type==rterminal2Low);
}

inline Boolean isRInternalExon(StateType type){
    return (type==rinternal0High || type==rinternal1High || type==rinternal2High || type==rinternal0Medium || type==rinternal1Medium || type==rinternal2Medium || type==rinternal0Low || type==rinternal1Low || type==rinternal2Low);
}


inline Boolean isFirstExon(StateType type){
    return (isInitialExon(type) || isRTerminalExon(type)
    		|| type==singleGHigh || type==singleGMedium || type==singleGLow
    		|| type==rsingleGHigh || type==rsingleGMedium || type==rsingleGLow);
}

inline Boolean isLastExon(StateType type){
    return (type==terminalHigh || type==terminalMedium || type==terminalLow
    		|| type==rinitialHigh || type==rinitialMedium || type==rinitialLow
    		|| type==singleGHigh || type==singleGMedium || type==singleGLow
    		|| type == rsingleGHigh || type == rsingleGMedium || type == rsingleGLow);
}

inline Boolean is5UTR(StateType type){
    return (utr5singleHigh <= type && type <= utr5termLow)
	|| (rutr5singleHigh <= type && type <= rutr5termLow);
}
...
inline Boolean is3UTR(StateType type){
    return  (utr3singleHigh <= type && type <= utr3termLow)
	|| (rutr3singleHigh <= type && type <= rutr3termLow);
}
...
inline Boolean isCodingExon(StateType type) {
    return  (singleGHigh <= type && type <= terminalLow)
	|| (rsingleGHigh <= type && type <= rterminal2Low);
}
...
inline Boolean isOnFStrand(StateType type){
    return (type >= singleGHigh && type <= terminalLow) || (type >= lessD0 && type <= utr3termLow);
}

inline StateType initialExonHigh(int frame){
    frame = mod3(frame);
    if (frame == 0)
	return initial0High;
    if (frame == 1)
...
inline StateType initialExonMedium(int frame){
    frame = mod3(frame);
    if (frame == 0)
	return initial0Medium;
    if (frame == 1)
	return initial1Medium;
    if (frame == 2)
	return initial2Medium;
    return TYPE_UNKNOWN;
}

inline StateType initialExonLow(int frame){
    frame = mod3(frame);
    if (frame == 0)
	return initial0Low;
    if (frame == 1)
...
inline StateType internalExonHigh(int frame){
    frame = mod3(frame);
    if (frame == 0)
	return internal0High;
    if (frame == 1)
	return internal1High;
    if (frame == 2)
	return internal2High;
    return TYPE_UNKNOWN;
}

inline StateType internalExonMedium(int frame){
    frame = mod3(frame);
    if (frame == 0)
	return internal0Medium;
    if (frame == 1)
	return internal1Medium;
    if (frame == 2)
	return internal2Medium;
    return TYPE_UNKNOWN;
}

inline StateType internalExonLow(int frame){
    frame = mod3(frame);
    if (frame == 0)
	return internal0Low;
    if (frame == 1)
...
inline StateType rterminalExonHigh(int frame){
    frame = mod3(frame);
    if (frame == 0)
	return rterminal0High;
    if (frame == 1)
	return rterminal1High;
    if (frame == 2)
	return rterminal2High;
    return TYPE_UNKNOWN;
}

inline StateType rterminalExonMedium(int frame){
    frame = mod3(frame);
    if (frame == 0)
	return rterminal0Medium;
    if (frame == 1)
	return rterminal1Medium;
    if (frame == 2)
	return rterminal2Medium;
    return TYPE_UNKNOWN;
}

inline StateType rterminalExonLow(int frame){
    frame = mod3(frame);
    if (frame == 0)
	return rterminal0Low;
    if (frame == 1)
	return rterminal1Low;
    if (frame == 2)
	return rterminal2Low;
    return TYPE_UNKNOWN;
}

inline StateType rinternalExonHigh(int frame){
    frame = mod3(frame);
    if (frame == 0)
	return rinternal0High;
    if (frame == 1)
	return rinternal1High;
    if (frame == 2)
	return rinternal2High;
    return TYPE_UNKNOWN;
}

inline StateType rinternalExonMedium(int frame){
    frame = mod3(frame);
    if (frame == 0)
	return rinternal0Medium;
    if (frame == 1)
	return rinternal1Medium;
    if (frame == 2)
	return rinternal2Medium;
    return TYPE_UNKNOWN;
}

inline StateType rinternalExonLow(int frame){
    frame = mod3(frame);
    if (frame == 0)
	return rinternal0Low;
    if (frame == 1)
	return rinternal1Low;
    if (frame == 2)
	return rinternal2Low;
    return TYPE_UNKNOWN;
}
