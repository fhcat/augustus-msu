...
sub train{
	...
	if($format eq "gb"){ # if it has Genbank format, simply make a link under directory $workDir/training/ 
	    ...
	    print "2 The input training set file has protin FASTA format. Converting to Genbank format...\n" if ($verbose>=2);
	...
	system("etraining --lowT=0.47 --highT=0.63 --species=$species training.gb.train 1>train.out 2>train.err")==0 or die("failed to execute: $!\n");
	print " Finished!\n" if ($verbose>=2);
	...
	chdir "$workDir/training" or die ("Could not change directory to $workDir/training\n");
	$cmdString = "etraining --lowT=0.47 --highT=0.63 --species=$species training.gb.train 1>train.withoutCRF.out 2>train.withoutCRF.err";
	...
	chdir "$workDir/training" or die ("Could not change directory to $workDir/training\n");
	$cmdString="etraining --lowT=0.47 --highT=0.63 --species=$species training.gb.train --CRF=1 1>train.CRF.out 2>train.CRF.err";
...
sub trainWithUTR{
    ...
    $perlCmdString = "etraining --lowT=0.47 --highT=0.63 --species=$species --UTR=on train.all.gb > etrain.out";
