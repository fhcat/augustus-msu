...
$usage .= "--kfold=k                Make a k-fold cross validation (default: 10)\n";
...
#if (!opendir(TMPDIR, $optdir)){
#    system("mkdir $optdir");
#} else {
#    print "Directory $optdir already exists. Please rename or delete it.\n";
#    exit;
#}
...
my  $species_cfg_filename = $configdir . "species/$cmdpars{'species'}/" . $cmdpars{'species'} . "_parameters.cfg";
if ($cmdpars{'opt_trans_matrix'} eq ''){
...
my @snsp = evalsnsp(@curoptmeta, 0.5, 0.5);
print "\n";
...
if ($cmdpars{'opt_trans_matrix'} eq ''){
    ...
    for (my $lowT=0.1; $lowT<0.9; $lowT+=0.1) {
    for (my $highT=0.1; $highT<0.9; $highT+=0.1) {
	$found_improvement = 0;
...
if ($cmdpars{'opt_trans_matrix'} eq ''){
    ...
    print "Line 594\n";
    system("etraining --lowT=0.47 --highT=0.63 --species=$cmdpars{'species'} --AUGUSTUS_CONFIG_PATH=$configdir $be_silent $optdir/curtrain $modelrestrict");
}
...
sub evalsnsp {
	...
	if ($cmdpars{'opt_trans_matrix'} eq '') {# No need to retrain if the trans matrix is optimized.
        print "Line 646\n";
	    system("$cmdpars{'aug_exec_dir'}etraining --lowT=$lowT --highT=$highT --species=$cmdpars{'species'} --AUGUSTUS_CONFIG_PATH=$configdir $argument $pars $be_silent $modelrestrict $optdir/curtrain");
	}
