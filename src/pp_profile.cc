 ...
 *                      of 30% of the full block length is considered a
 *                      valid part. Returns -1 if block threshold is not
 *                      reached.
 */
void Block::bestPartialLogScore(bool complement, int dna_offset, PartScoreType& result) const {
    int part_size = int(0.3 * size());
    if (part_size < MIN_BLOCKSIZE) 
	part_size = MIN_BLOCKSIZE;
    string aa_seq;
    ...
    LLDouble winresult = 1;
    for (int t=0; t<part_size; t++) 
	winresult *= columns[t].Q(aa_seq[t]);
    LLDouble fullscore = winresult;
//    result.from = result.to = 0;
    for (int t=part_size; t<size(); t++) 
	fullscore *= columns[t].Q(aa_seq[t]);
 //    if (qscore < winresult) {
// 	result.to = part_size;
// 	result.score = winresult;
//     } else if (qscore > getThreshold()) {
// 	result.to = size();
// 	result.score = qscore;
//     }
    LLDouble best_partial = winresult;
    result.from = 0;
    result.to = part_size;
    result.score = -1;
    for (int t=part_size; t<size(); t++) {
	winresult *= columns[t].Q(aa_seq[t]);
	winresult /= columns[t-part_size].Q(aa_seq[t - part_size]);
	if (winresult > best_partial) {
	    result.to = t+1;
	    result.from = result.to - part_size;
	    best_partial = winresult;
	}
    }
    int from = result.from;
    int to = result.to;
    winresult = best_partial;
    if (best_partial >= getPartialThresh(false, from, to)) {
	do {
	    if (--from < 0) break;
	    winresult *= columns[from].Q(aa_seq[from]);
	    if (winresult < getPartialThresh(false, from, to))
		break;
	    if (winresult > best_partial) {
		result.from = from;
		best_partial = winresult;
	...
	do {
	    if (to == size()) break;
	    winresult *= columns[to].Q(aa_seq[to]);
	    if (winresult < getPartialThresh(false, result.from, ++to))
		break;
	    if (winresult > result.score) {
		result.to = to;
		best_partial = winresult;
	    }
	} while (1);
	if (fullscore > getThreshold() && fullscore > best_partial) {
	    result.from = 0;
	    result.to = size();
	    result.score = fullscore.log();
	} else
	    result.score = best_partial.log();
   } else {
	result.from = 0;
	if (fullscore > getThreshold()) {
	    result.to = size();
	    result.score = fullscore.log();
	} else 
	    result.to = 0;
