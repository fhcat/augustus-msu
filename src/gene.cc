...
void State::setTruncFlag(int end, int predEnd, int dnalen){
    if (end == dnalen-1 && (isInitialExon(type) || isInternalExon(type) ||
			    ...
			    isIntron(type) || type == utr3singleHigh
				|| type == utr3singleMedium
				|| type == utr3singleLow
				|| type == utr3termHigh
				|| type == utr3termMedium
				|| type == utr3termLow))
	truncated |= TRUNC_RIGHT;
    if ((predEnd == -1 || predEnd == 0 ) && (isInternalExon(type) || type == terminalHigh || type == terminalMedium || type == terminalLow ||
					       isRInternalExon(type) || type == rinitialHigh || type == rinitialMedium || type == rinitialLow || 
					       isCodingIntron(type) || 
...
Gene* StatePath::projectOntoGeneSequence(const char *genenames){
	...
	if (curState != NULL) {
		...
		while (curState && is5UTR(curState->type)){ // read in the 5' UTR
		    if (!last5utrexon)
			aGene->complete5utr = (curState->type == utr5singleHigh || curState->type == utr5singleMedium || curState->type == utr5singleLow || curState->type == utr5initHigh || curState->type == utr5initMedium || curState->type == utr5initLow);
		...
		while (curState && is3UTR(curState->type)){ // read in the reverse 3' UTR
		    if (!last3utrexon)
			aGene->complete3utr = (curState->type == rutr3singleHigh ||
					curState->type == rutr3singleMedium ||
					curState->type == rutr3singleLow || curState->type == rutr3termHigh
					|| curState->type == rutr3termMedium
					|| curState->type == rutr3termLow);
		    if (isExon(curState->type)){
			utrexon = curState->getBiologicalState();
			if (!last3utrexon)
		...
		if (curState->type == singleGHigh || curState->type == singleGMedium || curState->type == singleGLow || curState->type == rsingleGHigh || curState->type == rsingleGMedium || curState->type == rsingleGLow) {
		    ...
		    if (curState->type == terminalHigh || curState->type == terminalMedium || curState->type == terminalLow || curState->type == rinitialHigh || curState->type == rinitialMedium || curState->type == rinitialLow){
			aGene->exons = curState->getBiologicalState();
			...
			if (aGene->strand == plusstrand){
			    ...
			    aGene->frame = mod3(aGene->exons->frame() + aGene->exons->length());
			curState = curState->next;
			while(curState && curState->type != terminalHigh && curState->type != terminalMedium && curState->type != terminalLow && curState->type != rinitialHigh && curState->type != rinitialMedium && curState->type != rinitialLow) {
		...
		if (curState) {
			...
			while (curState && is5UTR(curState->type)){ // read in the reverse 5' UTR
				...
				aGene->complete5utr = (curState->type==rutr5singleHigh ||
						curState->type==rutr5singleMedium ||
						curState->type==rutr5singleLow ||
						curState->type==rutr5initHigh ||
						curState->type==rutr5initMedium ||
						curState->type==rutr5initLow);
			    if (isExon(curState->type)){
				utrexon = curState->getBiologicalState();
				if (!last5utrexon)
				    aGene->utr5exons = utrexon;
			...
			while (curState && is3UTR(curState->type)){ // read in the 3' UTR
				...
				aGene->complete3utr = (curState->type == utr3singleHigh ||
						curState->type == utr3singleMedium ||
						curState->type == utr3singleLow ||
						curState->type == utr3termHigh ||
						curState->type == utr3termMedium ||
						curState->type == utr3termLow);
			    if (isExon(curState->type)){
				utrexon = curState->getBiologicalState();
				if (!last3utrexon)
				    aGene->utr3exons = utrexon;
...
StatePath* StatePath::getInducedStatePath(Gene *genelist, int dnalen){
	...
	if (endOfPred < end || endOfPred == 0) {// have intergenic gap or first gene
		...
		path->push(new State(endOfPred+1, end, onFStrand? utr5singleHigh: rutr3singleHigh));
		endOfPred = end;
	    }
	    /*
		...
		path->push(new State(endOfPred+1, end, onFStrand? utr5initHigh: rutr3termHigh));
		endOfPred = end;
		while (exon->next) {
		    // UTR intron
			...
			path->push(new State(endOfPred+1, end, onFStrand? utr5internalHigh: rutr3internalHigh));
			endOfPred = end;
		    } else {
			// rightmost UTR exon
			end = exon->end - (onFStrand? Constant::trans_init_window : 0);
			path->push(new State(endOfPred+1, end, onFStrand? utr5termHigh: rutr3initHigh));
			endOfPred = end;
		    }
		}
	    }
		...
		path->push(new State(endOfPred+1, end, onFStrand? singleGHigh: rsingleGHigh));
		endOfPred = end;
	    }
	    /*
	     * ... or multi CDS gene?
		...
		path->push(new State(endOfPred+1, end, onFStrand? initialExonHigh(frame) : rterminalExonHigh(frame)));
		endOfPred = end;
		...
		for (exon = curgene->exons->next; exon->next != NULL; exon = exon->next){
		    ...
		    path->push(new State(endOfPred+1, end, onFStrand? internalExonHigh(frame) : rinternalExonHigh(frame)));
		    endOfPred = end;
		}
		...
		path->push(new State(endOfPred+1, end, onFStrand? terminalHigh : rinitialHigh));
		endOfPred = end;
	    }
		...
		path->push(new State(endOfPred+1, end, onFStrand? utr3singleHigh: rutr5singleHigh));
		endOfPred = end;
	    }
	    /*
		...
		path->push(new State(endOfPred+1, end, onFStrand? utr3initHigh: rutr5termHigh));
		endOfPred = end;
		while (exon->next) {
		    // UTR intron
		    end = exon->next->begin - 1 - (onFStrand? Constant::ass_upwindow_size + Constant::ass_start + ASS_MIDDLE : Constant::dss_end + DSS_MIDDLE);
			...
			path->push(new State(endOfPred+1, end, onFStrand? utr3internalHigh: rutr5internalHigh));
			endOfPred = end;
		    } else {
			// rightmost UTR exon
			end = exon->end + (onFStrand? 0 : Constant::tss_upwindow_size);
			path->push(new State(endOfPred+1, end, onFStrand? utr3termHigh: rutr5initHigh));
			endOfPred = end;
		    }
		}
	    }
...
bool Gene::completeCDS() const {
	...
	exons->type == terminalHigh || exons->type == terminalMedium || exons->type == terminalLow || isRTerminalExon(lastExon()->type) ||
	exons->type == rinitialHigh || exons->type == rinitialMedium || exons->type == rinitialLow || isInitialExon(lastExon()->type))
...
void Gene::printGFF() const{
    ...
    // die Str��nge als + angegeben.
    if (exons) {
	// output forward strand start codon
	if (print_start && (strand == plusstrand) && (isInitialExon(exons->type) || exons->type == singleGHigh || exons->type == singleGMedium || exons->type == singleGLow)){
	...
	if (print_stop && (strand == minusstrand) && 
	    (exons->type == terminalHigh || exons->type == terminalMedium || exons->type == terminalLow || exons->type == singleGHigh || exons->type == singleGMedium || exons->type == singleGLow || isRTerminalExon(exons->type) || exons->type == rsingleGHigh || exons->type == rsingleGMedium || exons->type == rsingleGLow)){
	...
	if (print_exonnames && !gff3) {
	    cout << seqname << "\t" << source << "\t";
	    if (curExon->type == singleGHigh || curExon->type == singleGMedium || curExon->type == singleGLow || curExon->type == rsingleGHigh || curExon->type == rsingleGMedium || curExon->type == rsingleGLow)
		cout << "single";
	    else if (isInitialExon(curExon->type) || curExon->type == rinitialHigh || curExon->type == rinitialMedium || curExon->type == rinitialLow)
		cout << "initial";
	    else if (curExon->type == terminalHigh || curExon->type == terminalMedium || curExon->type == terminalLow ||isRTerminalExon(curExon->type))
		cout << "terminal";
	    else 
		cout << "internal";
	    cout << "\t" 
	...
	if (print_cds) {
		...
		if (curExon->type == terminalHigh || curExon->type == terminalMedium || curExon->type == terminalLow || curExon->type == singleGHigh || curExon->type == singleGMedium || curExon->type == singleGLow)
		    endmod = -3;
		if (isRTerminalExon(curExon->type) || curExon->type == rsingleGHigh || curExon->type == rsingleGMedium || curExon->type == rsingleGLow)
		    beginmod = 3;
	    }
	...
	if (print_stop && (strand == plusstrand) && (lastExon->type == terminalHigh || lastExon->type == terminalMedium || lastExon->type == terminalLow || lastExon->type == singleGHigh || lastExon->type == singleGMedium || lastExon->type == singleGLow)){
	    cout << seqname << "\t" << source << "\t" << "stop_codon" << "\t"
	...
	if (print_start && (strand == minusstrand) && 
	    (isInitialExon(lastExon->type) || lastExon->type == singleGHigh || lastExon->type == singleGMedium || lastExon->type == singleGLow || lastExon->type == rinitialHigh || lastExon->type == rinitialMedium || lastExon->type == rinitialLow || lastExon->type == rsingleGHigh || lastExon->type == rsingleGMedium || lastExon->type == rsingleGLow)){
	    cout << seqname << "\t" << source << "\t" << "start_codon" << "\t"
		 << lastExon->end - 1 << "\t" << lastExon->end + 1
		 << "\t.\t-\t0\t" << parentstr << endl;
	}
...
void reverseGeneSequence(Gene* &seq, int endpos){
	...
	while (headst) {
	    ...
	    switch (headst->type) {
		case initial0High : case initial0Medium : case initial0Low : case initial1High: case initial1Medium: case initial1Low: case initial2High : case initial2Medium : case initial2Low:
		    headst->type = rinitialHigh; headst->type = rinitialMedium; headst->type = rinitialLow; break;
		case terminalHigh : case terminalMedium : case terminalLow : 
		    headst->type = rterminalExonHigh(mod3(2-headst->length())); headst->type = rterminalExonMedium(mod3(2-headst->length())); headst->type = rterminalExonLow(mod3(2-headst->length())); break;
		case internal0High : case internal0Medium : case internal0Low : case internal1High : case internal1Medium : case internal1Low : case internal2High : case internal2Medium : case internal2Low:
		    headst->type = rinternalExonHigh(mod3(2+headst->frame() - headst->length())); headst->type = rinternalExonMedium(mod3(2+headst->frame() - headst->length())); headst->type = rinternalExonLow(mod3(2+headst->frame() - headst->length()));break;
		case singleGHigh : case singleGMedium : case singleGLow : 
		    headst->type = rsingleGHigh; headst->type = rsingleGMedium; headst->type = rsingleGLow; break;
