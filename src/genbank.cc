...
AnnoSequence* GBProcessor::getAnnoSequence( GBPositions* pos ){
	...
	while( st ){
	  ...
	  if (st == gene->exons)
	    if (st->next == NULL)
	      st->type =  (fit->strand == plusstrand)? singleGHigh : rsingleGHigh;
	    else
	      if (fit->strand == plusstrand)
		st->type = initialExonHigh(gene->clength);
	      else 
		st->type = rterminalExonHigh(2 - gene->clength);
	  else if (st->next == NULL)
	    if (fit->strand == plusstrand)
	      st->type = terminalHigh;
	    else 
	      st->type = rinitialHigh;
	  else 
	    if (fit->strand == plusstrand)
	      st->type = internalExonHigh(gene->clength);
	    else 
	      st->type = rinternalExonHigh(2 - gene->clength);
	  
	  if (stopCodonExcludedFromCDS) {
	      if (st->type == singleGHigh || st->type == terminalHigh){
		  	    gene->clength += 3;
			    st->end += 3;
	      }
	      if (st->type == rsingleGHigh || isRTerminalExon(st->type)){
		  gene->clength += 3;
		  st->begin -= 3;
	      }
	  }
