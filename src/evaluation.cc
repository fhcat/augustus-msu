...
void Evaluation::addToEvaluation(Gene *predictedGenes, Gene *annotatedGenes, AnnoSequence *annoseq, Strand strand, Double quotient){
    // find flanking regions on both sides (any strand)
    leftFlankEnd=-1, rightFlankBegin=-1;
    for (Gene* a = annotatedGenes; a != NULL; a = a->next){
	if ((leftFlankEnd == -1 && a->geneBegin() > 0) || (leftFlankEnd >=0 && a->geneBegin() - 1 < leftFlankEnd))
    ...
    annoBW = Gene::getGenesOnStrand(annotatedGenes, minusstrand);
    if (strand == bothstrands || strand == plusstrand) {
	addToEvaluation(predFW, annoFW, annoseq);
    }
    if (strand == bothstrands || strand == minusstrand) {
	addToEvaluation(predBW, annoBW, annoseq);
    }
...
void Evaluation::addToEvaluation(Gene* predictedGeneList, Gene* annotatedGeneList, AnnoSequence *annoseq){
#ifdef DEBUG
    // for analysis only
    if (!predictedGeneList && annotatedGeneList)
	cerr << "In strand containing " << annotatedGeneList->id << " nothing was found." << endl;
...
void Evaluation::print(int length){
    cout << "\n*******      Evaluation of gene prediction     *******" << endl << endl;

    nukTN = length - (nukTP+nukFP+nukFN);

	double numerator = (nukTP/1000.0*nukTN/1000.0)-(nukFN/1000.0*nukFP/1000.0);
	long double d2 = (nukTP/1000.0+nukFN/1000.0) * (nukTN/1000.0+nukFP/1000.0) * (nukTP/1000.0+nukFP/1000.0) * (nukTN/1000.0+nukFN/1000.0);
	double denominator = sqrt(d2);
	nukCC = numerator / denominator;
    ...
    cout << setw(16) << "nucleotide level" << " | " << setw(11)
	 ...
	 << " | " <<  setw(11) << setprecision(3) << nukSpec
    << " | " <<  setw(11) << setprecision(3) << nukCC
	<< " | " <<  setw(11) << setprecision(3) << nukTN
	<< " | " <<  setw(11) << setprecision(3) << nukTP
	<< " | " <<  setw(11) << setprecision(3) << nukFP
	<< " | " <<  setw(11) << setprecision(3) << nukFN
	<< endl;
    cout << "---------------------------------------------/" << endl << endl;
