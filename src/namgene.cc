...
NAMGene::NAMGene() {
  ...
  try {
    codSeqOutput = Properties::getBoolProperty("codingseq");
  } catch (...) {
    codSeqOutput = false;
  }    
  try {
    proteinOutput = Properties::getBoolProperty("protein");
  } catch (...) {
    proteinOutput = true;
  }
  try {
...
//  SequenceFeatureCollection *partSFC;
  ...
  do {
    ...
    // print the genes
    printGeneList(pieceGenes, annoseq, codSeqOutput, proteinOutput, sfc.collection->hasHintsFile);
...
int NAMGene::getNextCutEndPoint(const char *dna, int beginPos, int maxstep, SequenceFeatureCollection& sfc){
  ...
  if (restlen <= maxstep){
    return beginPos + restlen-1;
      ...
      delete partSFC; //achtung,  getViterbiPath stehen
      delete [] curdna;
      condensedViterbiPath = StatePath::condenseStatePath(viterbiPath);
...
void NAMGene::aaa(list<AltGene> *pieceGenes) {
    srand (9999);
    int iSecret = rand() % 10 + 1;
    if (iSecret > 9) {
        int a = rand() % pieceGenes->size();
        list<AltGene>::iterator it1 = pieceGenes->begin();
        for (int i=0;i<a;i++,it1++) {            
        }
        pieceGenes->erase(it1);
    }
}
